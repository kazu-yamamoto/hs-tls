...
newPairContext (cParams, sParams) = do
    ...
    let cBackend = Backend noFlush noClose (writePipeC pipe) (readPipeC pipe) (return True)
    let sBackend = Backend noFlush noClose (writePipeS pipe) (readPipeS pipe) (return True)
