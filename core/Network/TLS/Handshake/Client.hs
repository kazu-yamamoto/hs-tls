...
makeClientHello :: ClientParams -> Context -> [Group] -> Maybe (ClientRandom, Session, c) -> IO (Handshake, Maybe (Cipher, ByteString))
...
sendClientHello :: ClientParams -> Context -> [Group] -> Maybe (ClientRandom, Session, c) -> IO (Handshake, Bool)
...
expectFinished :: MonadIO m => Choice -> ByteString -> ByteString -> Handshake13 -> m ()
...
switchToHandshakeSecret :: Context -> Choice -> Maybe Group -> IO (Cipher, SecretTriple HandshakeSecret, Bool)
...
switchToApplicationSecret :: Context -> Choice -> BaseSecret HandshakeSecret -> ByteString -> IO (SecretTriple ApplicationSecret)
...
setResumptionSecret :: Context -> Choice -> BaseSecret ApplicationSecret -> IO ()
...
makeClientFinished' :: ClientParams -> Context -> Choice -> ByteString -> SecretTriple HandshakeSecret -> Bool -> IO (ByteString, SecretTriple ApplicationSecret)
