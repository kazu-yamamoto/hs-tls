...
data ECDHPrivate = ECDHPri_P256 (Scalar Curve_P256R1)
                 ...
                 | ECDHPri_X448 (Scalar Curve_X448)
                 deriving (Eq, Show)
...
ecdhGenerateKeyPair X448 =
    (ECDHPri_X448,ECDHPub_X448) `fs` curveGenerateKeyPair x448
...
ecdhGetPubShared (ECDHPub_X448 pub) =
    first ECDHPub_X448 <$> deriveEncrypt x448 pub
...
ecdhGetShared (ECDHPub_X448 pub) (ECDHPri_X448 pri) = deriveDecrypt x448 pub pri
...
encodeECDHPublic (ECDHPub_X448 p) = (X448, encodePoint x448 p)
...
decodeECDHPublic X448 bs = eitherCryptoError $ ECDHPub_X448 <$> decodePoint x448 bs
...
ecdhPrivateGroup (ECDHPri_X448 _) = X448
